...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    adminbruger.comparePassword(password, function (err, isMatch) {
      ...
      if (isMatch) {
        req.session.userId = 'adminbruger._id'; // LAV SESSION med brugers id

        res.json({
          brugernavn: adminbruger.brugernavn,
          brugerID: adminbruger._id,
        });
      } else {
        // slet cookie - så en evt. tidligere succes bliver slettet når man forsøger at logge ind med forkert
        res
          ...
          .json({ message: '2. Bruger findes ikke' });
      }
    });
  } catch (err) {
